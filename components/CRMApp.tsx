...
function CRMAppContent({ sessionEmail, onLogout }: { sessionEmail: string; onLogout: () => void }) {
  ...
  async function copyLocalDataToSupabase() {
    const confirmed = window.confirm(
      "Copier les données locales vers Supabase ? Cette action n’efface rien dans le navigateur."
    );
    ...
    if (authError || !authData.user) {
      notify("Connexion Supabase introuvable. Reconnecte-toi puis réessaie.", "warning");
      return;
    }
    ...
    try {
      await upsertRows("crm_contacts", data.contacts.map((contact) => ({
        user_id: userId,
        id: contact.id,
        name: contact.name,
        kind: contact.kind,
        ...
        created_at: contact.createdAt || new Date().toISOString().slice(0, 10)
      })));

      await upsertRows("crm_leads", data.leads.map((lead) => ({
        user_id: userId,
        id: lead.id,
        category: lead.category,
        contact_name: lead.contactName,
        asset_type: lead.assetType ?? "",
        asset_id: lead.assetId ?? "",
        status: lead.status,
        value: lead.value ?? 0,
        priority: lead.priority,
        next_action: lead.nextAction ?? "",
        notes: lead.notes ?? "",
        due_date: dateOrNull(lead.dueDate),
        rental_start_date: dateOrNull(lead.rentalStartDate),
        rental_end_date: dateOrNull(lead.rentalEndDate)
      })));

      await upsertRows("crm_tasks", data.tasks.map((task) => ({
        user_id: userId,
        id: task.id,
        title: task.title,
        owner: task.owner,
      ...
      await upsertRows("crm_properties", data.properties.map((property) => ({
        user_id: userId,
        id: property.id,
        name: property.name,
        city: property.city,
      ...
      await upsertRows("crm_vehicles", (data.vehicles ?? []).map((vehicle) => ({
        user_id: userId,
        id: vehicle.id,
        name: vehicle.name,
        brand: vehicle.brand,
      ...
      await upsertRows("crm_boats", (data.boats ?? []).map((boat) => ({
        user_id: userId,
        id: boat.id,
        name: boat.name,
        port: boat.port,
      ...
      await upsertRows("crm_quotes", savedQuotes.map((quote) => ({
        user_id: userId,
        id: quote.id,
        ...
        start_date: dateOrNull(quote.startDate),
        end_date: dateOrNull(quote.endDate),
        unit_price: quote.unitPrice ?? 0,
        validity_date: dateOrNull(quote.validityDate),
        payment_terms: quote.paymentTerms ?? "",
        cancellation_terms: quote.cancellationTerms ?? "",
      ...
      notify("Données copiées vers Supabase.");
    } catch (error) {
      const message = error instanceof Error ? error.message : "Erreur inconnue.";
      window.alert(`Copie vers Supabase impossible : ${message}`);
    }
  }
